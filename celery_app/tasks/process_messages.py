...
def _run_async(coro):
    ...
    try:
        ...
        # Уже внутри работающего event loop — используем nest_asyncio
        import nest_asyncio
        nest_asyncio.apply()
        return loop.run_until_complete(coro)
    except RuntimeError:
        # Нет запущенного event loop — просто запускаем новый
...
def process_target_buffer(chat_id: int, topic_id: int):
    """Обрабатывает буфер сообщений: по таймеру или при MAX_BATCH_SIZE."""
...
async def _process_target_buffer_async(chat_id: int, topic_id: int):
    ...
    if not messages:
        logger.debug("Buffer empty, skipping | chat=%s topic=%s", chat_id, topic_id)
        return {"tasks": 0, "meetings": 0, "status": "skipped"}

    logger.info("Processing buffer | chat=%s topic=%s size=%s", chat_id, topic_id, len(messages))

    processor = BatchProcessor()
    result = await processor.process_batch(chat_id, topic_id, messages)

    logger.info("Buffer done | chat=%s topic=%s tasks=%s meetings=%s",
                chat_id, topic_id, result["tasks_created"], result["meetings_created"])
